// Generates the extension's 4 PNG icons from NesisMark (SVG path).
//
// Run once (and again whenever the mark changes):
...
// Adds ~10% inner padding so the mark does not touch the edges at small sizes
// (16/32). Done by shrinking the drawable area with translate+scale.
